...
public class AbilityAimState : AbilityState
{
    ...
    public override void Enter()
    {
        ...
        {
            rightLauncher.localPosition = Vector3.Lerp(rightLauncher.localPosition, rightLauncher.localPosition + metricsManager.CurrentMetrics.AbilityData.AimBallOffset, 0.5f);
        }

        if (reusableData.LeftInput)
        {
            leftLauncher.localPosition = Vector3.Lerp(leftLauncher.localPosition, leftLauncher.localPosition + metricsManager.CurrentMetrics.AbilityData.AimBallOffset, 0.5f);
        }
    }
    ...
    private void ShowAiming()
    {
        ...
        if (aimObject == null)
        {
            aimObject = GameObject.Instantiate(metricsManager.CurrentMetrics.AbilityData.AimBall, aimEndPosition, Quaternion.identity);
        }
